...
- For a modified channel, the user's value **overrides the scripted state for 3 seconds** after the last touch (a hold time).
...
- **Reconciliation**: the exemplar's recipe, generalized to the schema — per-parameter modified flags, ~3 s hold, exponential blend back (`displayed = lerp(scripted, user, 0.92)` per frame), discrete revert after hold, audio pause only on scene-changing interaction.
...
| `@pause(prompt: "Essayez de déplacer le point.")` | Hard checkpoint: the clock halts, the prompt is shown, playback resumes on user action. A first-class version of the "try it yourself" moments the exemplar could only speak aloud. |
...
```json
{
  ...
  "pauses":   [ { "t": 29.75, "prompt": "Déplacez le point rouge…" } ]
}
```
