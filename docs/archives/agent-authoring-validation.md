...
- **#2 computed process** → the build-time, checkable `@bake` directive is implemented. Backprop now calls its real descent function at all three narration anchors and contains no pasted weight targets; see [the computed-process note](./computed-processes.md).
